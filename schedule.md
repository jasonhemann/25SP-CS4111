 ...
 <summary><strong>{{ week_start_seconds | date: "%b-%d" }} - {{ week_end_seconds | date: "%b-%d" }} </strong></summary>
  ...
  <li><strong>Assignments:</strong>
	<ul>
	  ...
	  <li><strong>{{ hw.title }}:</strong> Assigned on {{ out_day_seconds | date: '%a, %b %d' }}{% if hw.due_date %}; due {{ hw.due_date | date: '%a, %b %d' }}{% endif %}{% if hw.starter_code %} | <a href="{{ site.sourceurl }}{{ site.repository }}/tree/master/_starter_code/{{ hw.starter_code }}">Starter Code</a>{% endif %}</li>
	  {% endfor %}
	</ul>
 </li>
  {% if week.deliverables_due.size > 0 %}
  <li><strong>Due This Week:</strong>
    <ul>
      {% for item in week.deliverables_due %}
      {% assign due_day_offset = -1 %}
      {% for day in days %}
        {% if day == item.day %}
          {% assign due_day_offset = forloop.index0 %}
          {% break %}
        {% endif %}
      {% endfor %}
      {% assign due_day_seconds = due_day_offset | times: SECONDS_PER_DAY | plus: week_start_seconds %}
      <li><strong>{{ item.title }}:</strong> Due {{ due_day_seconds | date: '%a, %b %d' }}</li>
      {% endfor %}
    </ul>
  </li>
  {% endif %}
